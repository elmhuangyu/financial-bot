...
const filteredRisk = computed(() => {
  return risk.value.filter(
    (r: any) => r.Category === "Absolute Risk & Return" && keyMetrics.includes(r.Metric),
  );
});
...
const topContributors = computed(() => {
  return [...symbols.value]
    .sort(
      (a: any, b: any) => parseFloat(b.Contribution_Pct || 0) - parseFloat(a.Contribution_Pct || 0),
    )
    .slice(0, 6);
});
...
const topDetractors = computed(() => {
  return [...symbols.value]
    .sort(
      (a: any, b: any) => parseFloat(a.Contribution_Pct || 0) - parseFloat(b.Contribution_Pct || 0),
    )
    .slice(0, 6);
});
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div class="card bg-base-200 border border-base-300 p-6 shadow-sm">
      <div
        class="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-3 border-b border-base-300 gap-2"
      >
        <div>
          <h3 class="text-base font-bold text-white flex items-center gap-2">
            <BarChart3 class="w-5 h-5 text-secondary" /> Brinson-Fachler Multi-Period Attribution vs
            S&P 500
          </h3>
          <p class="text-xs text-slate-400">
            Frongello-smoothed Allocation (+27.63%) vs Selection (-1.58%)
          </p>
        </div>
          ...
          <tbody class="divide-y divide-base-300/60">
            <tr v-for="b in brinson" :key="b.Sector" class="hover:bg-base-300/40">
              ...
              <td
                ...
                :class="
                  parseFloat(b.AllocationEffect_Pct) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                "
              >
                {{ parseFloat(b.AllocationEffect_Pct) > 0 ? "+" : ""
                }}{{ parseFloat(b.AllocationEffect_Pct).toFixed(2) }}%
              </td>
              <td
                class="text-right font-semibold"
                :class="
                  parseFloat(b.SelectionEffect_Pct) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                "
              >
                {{ parseFloat(b.SelectionEffect_Pct) > 0 ? "+" : ""
                }}{{ parseFloat(b.SelectionEffect_Pct).toFixed(2) }}%
              </td>
              <td
                class="text-right font-bold"
                :class="
                  parseFloat(b.TotalAttribution_Pct) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                "
              >
                {{ parseFloat(b.TotalAttribution_Pct) > 0 ? "+" : ""
                }}{{ parseFloat(b.TotalAttribution_Pct).toFixed(2) }}%
              </td>
              <td class="text-right text-slate-300">
                {{ parseFloat(b.AccountContribution_Pct).toFixed(2) }}%
              </td>
              <td class="text-right text-slate-400">
                {{ parseFloat(b.BenchmarkContribution_Pct).toFixed(2) }}%
              </td>
              <td class="text-right font-bold text-secondary">
                {{ parseFloat(b.ContributionDifference_Pct) > 0 ? "+" : ""
                }}{{ parseFloat(b.ContributionDifference_Pct).toFixed(2) }}%
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card bg-base-200 border border-base-300 p-6 shadow-sm">
        <h3
          class="text-sm font-bold text-white flex items-center gap-2 mb-4 pb-2 border-b border-base-300"
        >
          <Sliders class="w-4 h-4 text-amber-400" /> CAPM & Fama Benchmark Factor Exposures
        </h3>
        <div class="space-y-3">
          <div
            class="p-3 rounded-xl bg-base-300/60 border border-base-300 flex justify-between items-center"
          >
            <div>
          ...
          >
            <div>
              ...
              <p class="text-[11px] text-slate-400">
                Active return per unit tracking risk (2.30% TE)
              </p>
            </div>
            ...
            <tbody class="divide-y divide-base-300/60">
              <tr v-for="r in filteredRisk" :key="r.Metric" class="hover:bg-base-300/40">
                ...
                <td class="text-right font-bold text-primary">
                  {{ isNaN(parseFloat(r.Account)) ? r.Account : parseFloat(r.Account).toFixed(2) }}
                </td>
                <td class="text-right text-slate-300">
                  {{
                    isNaN(parseFloat(r["SPXTR (S&P 500 TR)"]))
                      ? r["SPXTR (S&P 500 TR)"]
                      : parseFloat(r["SPXTR (S&P 500 TR)"]).toFixed(2)
                  }}
                </td>
                <td class="text-right text-slate-400">
                  {{
                    isNaN(parseFloat(r["VT (World)"]))
                      ? r["VT (World)"]
                      : parseFloat(r["VT (World)"]).toFixed(2)
                  }}
                </td>
              </tr>
            </tbody>
            ...
            <tbody class="divide-y divide-base-300/60">
              <tr v-for="s in topContributors" :key="s.Symbol">
                ...
                <td class="text-right font-bold text-emerald-400">
                  +{{ parseFloat(s.Contribution_Pct).toFixed(2) }}%
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table class="table table-xs w-full font-mono">
            ...
            <tbody class="divide-y divide-base-300/60">
              <tr v-for="s in topDetractors" :key="s.Symbol">
                ...
                <td
                  class="text-right"
                  :class="parseFloat(s.Return_Pct) >= 0 ? 'text-slate-300' : 'text-rose-400'"
                >
                  {{ parseFloat(s.Return_Pct).toFixed(1) }}%
                </td>
                <td class="text-right font-bold text-rose-400">
                  {{ parseFloat(s.Contribution_Pct).toFixed(2) }}%
                </td>
              </tr>
            </tbody>
