...
<template>
  <header class="sticky top-0 z-40 bg-base-200/80 backdrop-blur-md border-b border-base-300 px-4 lg:px-6 py-3">
    <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
      <!-- Left: Drawer Toggle & Brand -->
      <div class="flex items-center gap-3">
        <label for="drawer-runs" class="btn btn-ghost btn-sm btn-square lg:hidden" aria-label="Toggle Runs Drawer">
          <Menu class="w-5 h-5" />
        </label>

        <div class="p-2 bg-gradient-to-tr from-primary to-secondary rounded-xl shadow-md text-white">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400 hidden sm:block">Deterministic Analytics • Attribution & Asset Allocation</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <button 
          ...
          :class="isPrivacyMode ? 'btn-secondary shadow-sm shadow-secondary/20' : 'btn-outline border-base-300 text-slate-300'"
        ...
        <div class="join bg-base-300 p-0.5 rounded-lg border border-base-300">
          <button 
            ...
            :class="currentCurrency === 'USD' ? 'btn-primary text-white shadow' : 'btn-ghost text-slate-400'"
          >
            USD ($)
          </button>
          <button 
            ...
            class="join-item btn btn-xs font-semibold px-2.5 transition-all"
            :class="currentCurrency === 'CAD' ? 'btn-primary text-white shadow' : 'btn-ghost text-slate-400'"
          >
            CAD (C$)
          </button>
        </div>
        ...
        <div class="dropdown dropdown-end">
          <div tabindex="0" role="button" class="btn btn-ghost btn-sm btn-square" title="Select Theme">
            <Palette class="w-4 h-4 text-slate-400 hover:text-white" />
          </div>
          <ul tabindex="0" class="dropdown-content z-50 menu p-2 shadow-2xl bg-base-200 rounded-box w-36 border border-base-300 text-xs">
