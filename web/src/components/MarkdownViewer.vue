...
watch(() => props.files, (newFiles) => {
  ...
  if (markdownFiles.value.length > 0 && (!selectedFile.value || !markdownFiles.value.some(f => f.name === selectedFile.value))) {
...
<template>
  <div class="space-y-4">
    ...
    <div class="card bg-base-200 border border-base-300 p-4 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
      <div class="flex items-center gap-3 w-full sm:w-auto">
        <FileText class="w-5 h-5 text-primary" />
        <select v-model="selectedFile" class="select select-sm select-bordered bg-base-300 text-xs font-semibold rounded-lg w-full sm:w-72">
          <option v-for="f in markdownFiles" :key="f.name" :value="f.name">{{ f.name }}</option>
        </select>
      </div>

      <button @click="copyContent" class="btn btn-sm btn-outline border-base-300 gap-1.5 text-xs text-slate-300 hover:text-white">
