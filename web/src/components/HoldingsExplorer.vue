...
const ownerOptions = computed(() => {
  ...
  rawHoldings.value.forEach((h) => {
    if (h.owner) s.add(h.owner);
  });
  return Array.from(s);
});
...
const taxOptions = computed(() => {
  const s = new Set<string>();
  rawHoldings.value.forEach((h) => {
    if (h.tax_treatment) s.add(h.tax_treatment);
  });
  return Array.from(s);
});
...
const classOptions = computed(() => {
  const s = new Set<string>();
  rawHoldings.value.forEach((h) => {
    if (h.asset_class) s.add(h.asset_class);
  });
  return Array.from(s);
});
...
const processedHoldings = computed(() => {
  ...
  const filtered = rawHoldings.value.filter((h) => {
    ...
    if (searchQuery.value) {
      ...
      const text =
        `${h.symbol} ${h.asset_name} ${h.sector} ${h.account_label} ${h.industry}`.toLowerCase();
  ...
  return [...list].sort((a: any, b: any) => {
    ...
    if (
      ["market_value_usd", "unrealized_pl_usd", "quantity", "cost_basis_usd"].includes(
        sortKey.value,
      )
    ) {
        ...
        <table class="table table-xs w-full font-mono">
          <thead class="text-slate-400 bg-base-300/80 uppercase select-none font-sans">
            <tr>
              ...
              <th @click="toggleSort('asset_name')" class="cursor-pointer hover:text-white">
                Asset Name
              </th>
              <th @click="toggleSort('account_label')" class="cursor-pointer hover:text-white">
                Account(s)
              </th>
              <th @click="toggleSort('tax_treatment')" class="cursor-pointer hover:text-white">
                Tax Status
              </th>
              <th @click="toggleSort('sector')" class="cursor-pointer hover:text-white">Sector</th>
              <th
                @click="toggleSort('quantity')"
                class="text-right cursor-pointer hover:text-white"
              >
                Quantity
              </th>
              <th
                @click="toggleSort('market_value_usd')"
                class="text-right cursor-pointer hover:text-white font-bold text-white"
              >
                Market Value
              </th>
              <th
                @click="toggleSort('unrealized_pl_usd')"
                class="text-right cursor-pointer hover:text-white"
              >
                Unrealized P&L
              </th>
          ...
          <tbody class="divide-y divide-base-300/60">
            <tr
              v-for="h in processedHoldings"
              :key="h.symbol + (h.account_label || '')"
            ...
            >
              ...
              <td>
                <div v-if="h.is_aggregated && h.account_count > 1" class="flex items-center gap-1">
                  <span class="badge badge-xs badge-info font-bold"
                    >{{ h.account_count }} Accounts</span
                  >
                  <span
                    class="truncate max-w-[120px] text-[11px] text-slate-400"
                    :title="h.account_label"
                    >{{ h.account_label }}</span
                  >
                </div>
              ...
              <td>
                ...
                <span
                  ...
                  class="badge badge-xs font-semibold"
                  :class="
                    h.tax_treatment === 'Tax-Free'
                      ? 'badge-success badge-outline'
                      : h.tax_treatment === 'Tax-Deferred'
                        ? 'badge-warning badge-outline'
                        : 'badge-info badge-outline'
                  "
                >
              ...
              <td class="text-right text-slate-300">
                {{ maskNumber(parseFloat(h.quantity || 0).toLocaleString()) }}
              </td>
              <td class="text-right font-bold text-white">
                {{ formatMoney(h.market_value_usd, isPrivacyMode) }}
              </td>
              <td
                class="text-right font-semibold"
                :class="
                  parseFloat(h.unrealized_pl_usd || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'
                "
              >
                {{
                  isPrivacyMode
                    ? "••••••"
                    : (parseFloat(h.unrealized_pl_usd || 0) >= 0 ? "+" : "") +
                      formatMoney(h.unrealized_pl_usd)
                }}
              </td>
            </tr>
          </tbody>
          ...
          <tfoot class="bg-base-300 font-sans border-t border-base-300 font-bold text-white">
            <tr>
              <td colspan="5" class="py-3 px-4 text-xs text-slate-400">
                Showing {{ processedHoldings.length }}
                {{ isAggregateMode ? "consolidated assets" : "positions" }}
              </td>
