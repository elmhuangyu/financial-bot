...
1. Do NOT guess or invent arbitrary styling.
2. Activate and read the [`html-dashboard`](.agents/skills/html-dashboard/SKILL.md) skill.
3. Write an ad-hoc runner script in `data/tmp/generate_html_report.py` to compile the current session's deliverables into `data/output/index.html` (and `data/output/<report_name>.html`).
